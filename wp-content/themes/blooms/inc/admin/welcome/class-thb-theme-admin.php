...
class Thb_Theme_Admin {
	...
	public function thb_after_setup_theme() {
		...
		if ( ! is_admin() ) {
			add_filter( 'loop_shop_per_page', 'thb_ppp', 20 );

			function thb_ppp( $products_per_page ) {
				$products_per_page_get = filter_input( INPUT_GET, 'ppp', FILTER_VALIDATE_INT );
				$products_per_page     = isset( $products_per_page_get ) ? $products_per_page_get : $products_per_page;
		...
		// Text Domain.
		load_theme_textdomain( 'blooms', get_stylesheet_directory() . '/inc/languages' );
		...
		register_nav_menus(
			array(
				'nav-menu'     => esc_html__( 'Navigation Menu', 'blooms' ),
				'acc-menu-out' => esc_html__( 'Mobile - Secondary Menu (Not Logged-in)', 'blooms' ),
				'acc-menu-in'  => esc_html__( 'Mobile - Secondary Menu (Logged-in)', 'blooms' ),
			)
		);
		...
		$thb_sidebar_defaults = array(
			...
			'after_title'   => '</div><div class="thb-widget-content">',
		);

		$thb_sidebars = array(
			array(
				'name'        => esc_html__( 'Blog Sidebar', 'blooms' ),
				'id'          => 'blog',
				'description' => esc_html__( 'The sidebar in blog page', 'blooms' ),
			),
			array(
				'name'        => esc_html__( 'Article Sidebar', 'blooms' ),
				'id'          => 'single',
				'description' => esc_html__( 'The sidebar next to articles', 'blooms' ),
			),
			array(
				'name'        => esc_html__( 'Page Sidebar', 'blooms' ),
				'id'          => 'page',
				'description' => esc_html__( 'The sidebar on regular pages.', 'blooms' ),
			),
			array(
				'name'        => esc_html__( 'Mobile Menu Sidebar', 'blooms' ),
				'id'          => 'mobile-menu',
				'description' => esc_html__( 'You can also add widgets inside your mobile menu.', 'blooms' ),
			),
		);
		for ( $x = 1; $x <= 6; $x++ ) {
			$thb_sidebars[] = array(
				'name'        => esc_html__( 'Footer Column - ', 'blooms' ) . $x,
				'id'          => 'footer' . $x,
				'description' => esc_html__( 'Contents of the column #', 'blooms' ) . $x,
			);
		}
		...
		if ( thb_wc_supported() ) {
			array_unshift(
				$thb_sidebars,
				array(
					'name'        => esc_html__( 'Shop Sidebar', 'blooms' ),
					'id'          => 'thb-shop-filters',
					'description' => esc_html__( 'Sidebar used for filters on the Shop page', 'blooms' ),
				)
			);
		}
		// Register Sidebars.
	...
	public function thb_demos() {
		return array(
			array(
				'import_file_name'           => 'blooms',
				'import_file_url'            => 'https://themes.fuelthemes.net/theme-demo-files/blooms/democontent.xml',
				'import_widget_file_url'     => 'https://themes.fuelthemes.net/theme-demo-files/blooms/widget_data.json',
				'import_customizer_file_url' => 'https://themes.fuelthemes.net/theme-demo-files/blooms/customizer.dat',
				'import_image'               => self::$thb_theme_directory_uri . 'screenshot.jpg',
				'import_demo_url'            => 'https://blooms.fuelthemes.net/',
			),
		);
	}
	/**
	...
	public function thb_admin_page_enqueue( $hook_suffix ) {
		...
		wp_localize_script(
			...
			array(
				'i18n'           => array(
					'mediaTitle'  => esc_html__( 'Choose an image', 'blooms' ),
					'mediaButton' => esc_html__( 'Use image', 'blooms' ),
				),
	...
	public function thb_editor_styles() {
		...
		<?php if ( $accent_color ) { ?>
			.editor-styles-wrapper {
				--color-accent: <?php echo esc_html( $accent_color ); ?> !important;
			}
		<?php } ?>
		...
		<?php if ( $tertiary_color ) { ?>
			.editor-styles-wrapper {
				--color-tertiary: <?php echo esc_html( $tertiary_color ); ?> !important;
			}
		<?php } ?>
		<?php
		if ( $primary_font ) {
